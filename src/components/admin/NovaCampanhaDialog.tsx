...
import {
  ...
  Info, X, Loader2, ExternalLink, Lock,
} from "lucide-react"
...
function SearchableSelect<T>({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && !isLoading && (
        <div className="absolute z-[100] mt-1 w-full rounded-xl border border-[#1C1C2C] bg-[#0F0F1A] shadow-[0_8px_32px_rgba(0,0,0,0.6)]">
  ...
  if (instancias.length === 0) {
    return (
      <div className="flex items-center gap-2 p-4 bg-[#13131F] border border-[#1C1C2C] rounded-xl text-sm text-[#5A5A72]">
        ...
        <span>Nenhuma instância Z-API vinculada a este cliente. Adicione uma na aba Z-API do cliente.</span>
      </div>
    )
  }
