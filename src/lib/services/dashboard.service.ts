...
export async function getMetricas() {
  ...
  const [leadsHoje, leadsSemana, sucessoSemana, falhasSemana, queueStats, ultimosLeads] =
    await Promise.all([
  ...
  const taxaSucesso =
    processadosSemana > 0 ? Math.round((sucessoSemana / processadosSemana) * 1000) / 10 : 0
